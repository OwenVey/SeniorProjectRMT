...
class Users extends Component {
	...
	fetchUsers = async () => {
		...
		axios
			.get(url2)
			.then(response => {
				let users = response.data.message.users.map(user => {
					return {
						...user,
	...
	handleSave = () => {
		this.setState({
			userData: this.state.userData.map(user =>
				user.id === this.state.editUser.id ? Object.assign(this.state.editUser) : user
			),
			visible: false,
		});
	};
				...
				>
					...
					<p>
						...
						<Input
							...
							onChange={e =>
								this.setState({ editUser: { ...this.state.editUser, email: e.target.value } })
							}
							placeholder="Email"
						/>
					</p>
					<p>
						...
						<Input
							...
							onChange={e =>
								this.setState({ editUser: { ...this.state.editUser, userName: e.target.value } })
							}
					...
					<Switch
						...
						onChange={value => this.setState({ editUser: { ...this.state.editUser, userStatus: value } })}
					/>
					<span>Active</span>
				</Modal>
			</React.Fragment>
